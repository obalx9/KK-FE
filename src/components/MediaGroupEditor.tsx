...
export default function MediaGroupEditor({
  ...
  const migrateLegacyMedia = async () => {
    ...
    try {
      ...
      if (updateError) {
        console.error('[MediaGroupEditor] Error updating course_posts:', updateError);
      }
  ...
  const handleAddMedia = async (storagePath: string, fileSize: number, fileName: string) => {
    try {
      ...
      const { error: clearErrorFlag } = await supabase
        .from('course_posts')
        .update({
          has_error: false,
          error_message: null
      ...
      if (clearErrorFlag) {
        console.error('[MediaGroupEditor] Error clearing error flag:', clearErrorFlag);
      }
    } catch (error) {
  ...
  const handleDeleteMedia = async (mediaId: string) => {
    ...
    try {
      ...
      if (mediaId.startsWith('legacy-')) {
        ...
        if (error) {
          console.error('[MediaGroupEditor] Error deleting legacy media:', error);
          throw error;
        }
    ...
    try {
      ...
      await Promise.all(
        items.map(async (item, index) => {
          ...
          if (error) {
            console.error(`[MediaGroupEditor] Error reordering item ${item.id}:`, error);
            throw error;
          }
        })
      );
