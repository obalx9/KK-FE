...
const VIDEO_EXTENSIONS = /\.(mp4|mov|avi|mkv|webm|m4v|3gp|flv|wmv)$/i;

function isVideoFile(item: MediaItem): boolean {
  if (item.media_type === 'video' || item.media_type === 'animation') return true;
  if (item.file_name && VIDEO_EXTENSIONS.test(item.file_name)) return true;
  return false;
}
...
export default function MediaGallery({ items, courseId, onMediaClick, courseWatermark }: MediaGalleryProps) {
  ...
  const getSingleItemStyle = (item: MediaItem): React.CSSProperties => {
    if (item.width && item.height) {
      const ratio = item.width / item.height;
      const maxH = Math.min(500, Math.round(600 / ratio));
      return { minHeight: `${maxH}px` };
    }
  ...
  return (
    <div className={getContainerClass(sortedItems.length)}>
      {sortedItems.map((item, index) => {
        ...
        const hasThumbnail = isVideo && !!item.telegram_thumbnail_file_id;
        ...
        return (
          <div
            ...
            style={isSingle ? getSingleItemStyle(item) : undefined}
              ...
              <>
                ...
                {courseWatermark && (
                  <div
                    ...
                    style={{
                      textShadow: '1px 1px 2px rgba(0,0,0,0.9), -1px -1px 2px rgba(0,0,0,0.9), 0 0 4px rgba(0,0,0,0.8)',
