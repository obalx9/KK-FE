...
import { Search, Video, Calendar, Edit2, Trash2, X, Save, Upload, Plus, AlertTriangle, ArrowUp, ArrowDown, Pin, Megaphone, ExternalLink } from 'lucide-react';
...
export default function CourseFeed({
  ...
  const loadPinnedPosts = async () => {
    ...
    try {
      ...
      setPinnedPostIds(new Set((data || []).map(p => p.post_id)));
    } catch (error) {
  ...
  const loadAdsAndPremium = async () => {
    try {
      ...
      if (courseData?.seller_id) {
        const { data: sellerData } = await supabase
          .from('sellers')
          .select('premium_active, premium_expires_at')
          .eq('id', courseData.seller_id)
          .maybeSingle();

        const premiumValid = sellerData?.premium_active &&
          sellerData?.premium_expires_at &&
          new Date(sellerData.premium_expires_at) > new Date();
  ...
  const togglePinPost = async (postId: string) => {
    ...
    if (isPinned) {
      ...
      const { error } = await supabase
        .from('student_pinned_posts')
        .insert({ student_id: userId, course_id: courseId, post_id: postId });
  ...
  const loadPostsUntilFound = useCallback(async (postId: string) => {
    ...
    while (!found) {
      ...
      if (!editable) {
        query = query.eq('has_error', false);
      }

      const { data: postsData, error } = await query
        .order('published_at', { ascending: reverseOrder })
        .range(from, to);

      if (error || !postsData || postsData.length === 0) break;

      const postsWithMedia = await enrichPostsWithMedia(postsData);
    ...
    return found;
  }, [courseId, editable, courseSettings?.reverse_post_order]);
  ...
  }, [loadingMore, hasMore, searchQuery, page, courseId, editable, courseSettings?.reverse_post_order]);
