...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    ...
    if (user) {
      loadData();
      const key = `seller_docs_seen_${user.id}`;
  ...
  const dismissWelcomeBanner = () => {
    ...
    if (user) {
      localStorage.setItem(`seller_docs_seen_${user.id}`, '1');
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <div className="flex-shrink-0">
                <KeyKursLogo size={40} color="#0d9488" />
              </div>
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <a
                ...
                className="flex items-center gap-2 p-2 sm:px-3 sm:py-2 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
                title="Инструкции"
              >
                <FileText className="w-4 h-4" />
                <span className="hidden sm:inline text-sm font-medium">Инструкции</span>
              </a>
              {user?.roles.includes('student') && (
                <button
                  onClick={() => navigate('/student')}
                  ...
                  title="Перейти в режим студента"
                >
                  <GraduationCap className="w-4 h-4" />
                  <span className="hidden sm:inline text-sm font-medium">Режим студента</span>
                </button>
              )}
              <button
                onClick={handleSignOut}
        ...
        {showWelcomeBanner && (
          <div className="mb-6 bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-800 rounded-xl p-4 flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-teal-900 dark:text-teal-100 text-sm mb-0.5">Добро пожаловать в дашборд продавца!</p>
              <p className="text-sm text-teal-700 dark:text-teal-300">
                Ознакомьтесь с инструкциями — там описано, как создавать курсы, добавлять студентов и настраивать Telegram-бота.
              </p>
              <a
                href="/docs"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 mt-2 text-sm font-medium text-teal-700 dark:text-teal-300 hover:text-teal-900 dark:hover:text-teal-100 underline underline-offset-2 transition-colors"
              >
                <FileText className="w-4 h-4" />
                Открыть инструкции
              </a>
            ...
            <button
              ...
              className="flex-shrink-0 p-1.5 text-teal-500 hover:text-teal-700 dark:hover:text-teal-200 hover:bg-teal-100 dark:hover:bg-teal-900/50 rounded-lg transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
